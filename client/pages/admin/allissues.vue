<template>
  <v-card>
    <v-card-title>All Issues</v-card-title>
    <v-card-text>
      ...
      >
        ...
        <template v-slot:item.action="{ item }">
          ...
          <v-btn
            ...
            dark
            icon
            :to="{name:'admin.edit', params:{type:'issue', slug:item.slug}}"
          >
...
export default {
  ...
  methods: {
    ...
    async updateStatus(issue) {
      this.reset();
      try {
        await this.$api.put(`/issue/${issue.slug}`, { status: issue.status });
        this.fetchData();
    ...
    async deleteIssue(issue) {
      ...
      if (confirm("Are you sure?")) {
        try {
          await this.$api.delete(`/issue/${issue.slug}`);
          this.fetchData();
